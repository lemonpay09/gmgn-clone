...
export default function TradePage() {
  ...
  // useEffect 用于管理价格的 WebSocket 连接
  useEffect(() => {
    // 将 "BTC/USDT" 转换为 "btcusdt"
    const symbol = activePair.replace('/', '').toLowerCase();
    ...
    const connectPriceWebSocket = () => {
      try {
        ...
        ws.onopen = () => {
          console.log(`Price WebSocket connected for ${activePair}`);
        };

        ws.onmessage = (event) => {
          try {
            const tradeData = JSON.parse(event.data);
            // 'p' 字段是成交价格，只更新当前对应的币种价格
            setPrices(prev => ({
              ...prev,
              [activePair]: parseFloat(tradeData.p)
            }));
          } catch (error) {
        ...
        ws.onerror = (event) => {
          console.warn(`Price WebSocket error for ${activePair}`);
        };

        ws.onclose = (event) => {
          console.log(`Price WebSocket closed for ${activePair}`, {
            code: event.code,
            reason: event.reason,
          ...
          // 尝试重新连接（如果不是正常关闭）
          if (!event.wasClean) {
            console.log('Attempting to reconnect price WebSocket in 5 seconds...');
